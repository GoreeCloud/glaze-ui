...
- `contracts/v1.6/release-candidate.json` + `acceptance/v1.6.0-rc.1.md` + `js/glaze-v1.6.0-rc.1.mjs` + `scripts/verify_glaze_v1_6_release_candidate.mjs` — governed V1.6 Release Candidate identity and continuity control. The completed 24/24 evidence matrix is accepted for `1.6.0-rc.1`; the RC wrapper adds identity only over the byte-identical frozen-qualified Development aggregate, remains non-consumer-eligible, preserves V1.5.1 as current Stable, and cannot auto-grant Stable, deployment, production, tag, or GitHub Release status.
