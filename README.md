...
V1.6 extends the retained V1.5 Context + Capability Awareness foundation with the fully qualified V1.6 loading, state/accessibility, focus/motion, material/type/input, resilience/feedback, navigation/status, component-system, experience-governance, performance/diagnostics, conformance/adoption, and acceptance-control foundations. The shared qualification matrix is **24 verified / 0 unverified / 0 not applicable**.
...
- `acceptance/v1.6-stable-qualification-review.json` — final Stable qualification
- `acceptance/v1.6-final-security-acceptance.json` — final Stable security acceptance
- `acceptance/v1.6-publication-readback.json` — immutable publication/readback evidence
- `js/glaze-v1.6.0.mjs` — current Stable runtime entrypoint
- `consumers/registry.json` — current downstream target/adoption authority
- `scripts/verify_glaze_v1_6_stable_qualification_review.mjs` — fail-closed V1.6 Stable authority gate

The released source is `a7180679ea851389e0f3004515f9a25f420e716d`, published under immutable tag `v1.6.0` and GitHub Release `392095913`. The published archive, checksum manifest, CycloneDX SBOM, and provenance were read back byte-for-byte against the final-security-accepted artifact.

GLAZE UI `1.5.1` remains the immediate known-good Stable rollback baseline and preserved historical Stable provenance.

## Stable qualification and publication status

The governed V1.6 qualification matrix is complete at **24/24**, authoritative `main` is protected by active ruleset `23699829`, final Stable security acceptance passed, deterministic artifact/provenance evidence passed, immutable publication completed, and post-publication readback passed. The final Stable qualification review contains **0 blockers**.

This Stable classification applies to the shared GLAZE UI release only. It does **not** automatically grant any downstream GoreeCloud application V1.6 conformance, deployment acceptance, production acceptance, or production eligibility. Each consumer must complete fresh repository-local V1.6 adoption and acceptance.

## Retained V1.6 Release Candidate and Development provenance

The repository retains the superseded V1.6 Release Candidate together with its frozen Development and qualification inputs as immutable provenance beneath the current V1.6.0 Stable authority:
