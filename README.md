...
- `contracts/v1.6/qualification.human.plan.json` + `reference/v1.6/human-qualification.html` + `scripts/prepare_glaze_v1_6_human_qualification.py` — exact-source human-review control for Accessibility, Keyboard navigation, Privacy boundaries, Authority boundaries, and Representative rendering. The repository-local surface supports real keyboard/focus, semantic-state, privacy/authority, continuity, accessibility-mode, and representative-runtime observation but has no PASS control. A real reviewed session is now durably recorded at `acceptance/v1.6-human-evidence.json`; governed reconciliation of that accepted record with the canonical machine/rendered/regression evidence produces 22 verified / 2 unverified. Assistive Technology and Performance remain separate open lanes.
- `contracts/v1.6/qualification.assistive-technology.plan.json` + `reference/v1.6/assistive-technology-qualification.html` + `scripts/prepare_glaze_v1_6_assistive_technology_qualification.py` — exact-source assistive-technology review control for the separate Assistive Technology lane. It requires a real identified assistive-technology session, keyboard-or-equivalent navigation, dynamic-announcement observation, and separately recorded environment/findings; hosted CI validates only the control plane and cannot create assistive-technology evidence. A separately accepted assistive-technology record may also satisfy Accessibility's allowed human-or-assistive alternative group on the same frozen source.
- `acceptance/v1.6-human-evidence.template.json`, `acceptance/v1.6-assistive-technology-evidence.template.json`, and `acceptance/v1.6-performance-evidence.template.json` + their schemas + `scripts/validate_glaze_v1_6_external_evidence_records.mjs` — fail-closed durable-record controls for future real human, assistive-technology, and representative-performance evidence. Templates are explicitly non-evidence; accepted records must remove placeholders, satisfy exact-source review requirements, carry durable references, and still cannot promote lifecycle automatically.
- `contracts/v1.6/qualification-evidence-intake.json` + `scripts/evaluate_glaze_v1_6_qualification_evidence.mjs` — exact-source evidence reconciliation for the machine/rendered/regression baseline plus separately validated external records. Records counted by governed reconciliation must resolve by real path to regular files under repository `acceptance/`, be tracked by Git, and match the committed bytes at the exact reconciliation revision; absolute paths outside the repository, symlinks escaping that directory, untracked records, and working-tree drift fail closed. The standalone external-record validator may still preflight candidate records before they are durably stored. The no-external evaluation intentionally remains 17 verified / 7 unverified; including the accepted Human record now produces 22 verified / 2 unverified, leaving Assistive Technology and Performance open. `--require-complete` remains fail-closed until all section-98 evidence groups are satisfied. Even a complete matrix means only readiness for governed qualification review, never automatic Candidate, Release Candidate, Stable, consumer, deployment, or production acceptance.
