...
- `contracts/v1.6/qualification-evidence-intake.json` + `scripts/evaluate_glaze_v1_6_qualification_evidence.mjs` — exact-source evidence reconciliation for the current machine/rendered/regression records plus optional separately validated external records. Records counted by governed reconciliation must resolve by real path to regular files under repository `acceptance/`, be tracked by Git, and match the committed bytes at the exact reconciliation revision; absolute paths outside the repository, symlinks escaping that directory, untracked records, and working-tree drift fail closed. The standalone external-record validator may still preflight candidate records before they are durably stored. The default evaluation must reproduce the current 17 verified / 7 unverified disposition; `--require-complete` remains fail-closed until all section-98 evidence groups are satisfied. Even a complete matrix means only readiness for governed qualification review, never automatic Candidate, Release Candidate, Stable, consumer, deployment, or production acceptance.
