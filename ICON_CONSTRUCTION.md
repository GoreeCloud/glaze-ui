...
This contract defines how application and service identity assets are constructed, simplified, packaged, validated, and described for tooling across the V1 generation. It originated in GLAZE UI V1.0 and remains inherited by the current V1.4 Stable product unless a later governed contract explicitly supersedes it.
...
This construction contract originated in the official GLAZE UI V1.0 reset baseline, was inherited by V1.1, V1.2, and V1.3, and remains inherited by V1.4 where not superseded. The internal `1.5.0` icon-construction revision remains a subsystem revision and must not be interpreted as a Glaze UI product version. Machine-contract consistency does not certify rendered artwork or establish production acceptance. Fresh exact-revision visual, accessibility, optical-size, platform, and package validation remains consumer/asset specific. Human review deferred from the shared V1.4.0 design-system gate belongs to V1.4.1 and is not represented as passed evidence.
