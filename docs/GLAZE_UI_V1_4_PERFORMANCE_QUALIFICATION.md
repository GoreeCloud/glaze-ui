...
Glaze UI V1.4 now has a bounded source contract for performance qualification evidence. The contract exists to make later performance acceptance reproducible, exact-revision-bound, and time-bounded; it is not performance evidence by itself and it cannot promote V1.4.
...
This capability reduces one V1.4 release-readiness gap by making performance evidence machine-checkable and freshness-bounded. Stable qualification still requires real current measurements from governed target environments, accepted budgets and freshness policy, retained artifacts, reviewer acceptance, and the other independent Glaze lifecycle gates.
