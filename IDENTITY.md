...
- Canonical path: `systems/glaze-ui/glaze-ui-mark.svg`
- Local synchronized derivative: `assets/identity/official/facet/glaze-ui-mark.svg`

The Glaze UI repository retains the local SVG because its validators, generators, and Design Center build consume that path. The local file is a synchronized derivative and must not become an independent branding authority.
...
The exact approved local scalable derivative is:
...
- Source role: repository-synchronized derivative of the unified canonical artwork

The unified canonical source contains the exact reviewed Facet SVG promoted without geometry changes. Public and generated representations must trace back to it.
...
`GoreeCloud/goreecloud-branding-assets` path `systems/glaze-ui/glaze-ui-mark.svg` is the branding source of truth. Build systems may retain `assets/identity/official/facet/glaze-ui-mark.svg` as a byte-equivalent local consumer copy or produce deterministic derivatives from the canonical source. A derivative that cannot be traced to the unified source must not be represented as official Glaze UI artwork.
