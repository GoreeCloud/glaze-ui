#!/usr/bin/env python3
"""Validate preserved V1.3 release and V1.3.1 deferred-qualification provenance."""
...
def main():
    ...
    for path in [
        ...
        req((ROOT / path).is_file(), f"missing preserved V1.3/V1.3.1 authority: {path}")
    ...
    req(len(qrules) == 55 and {x.get("id") for x in qrules if isinstance(x, dict)} == RULE_IDS, "quality contract must retain exactly quality-01 through quality-55")
    ...
    req(
        "must not be represented as passed" in stable_acceptance or "not rewritten as a pass" in stable_acceptance,
        "Stable acceptance must preserve the no-fabricated-pass boundary",
    )
    ...
    print("GLAZE UI V1.3 historical release/deferred qualification provenance: PASS")
    print("V1.3.0 remains retained as Stable history; unresolved V1.3.1 evidence remains unmanufactured.")
    return 0
