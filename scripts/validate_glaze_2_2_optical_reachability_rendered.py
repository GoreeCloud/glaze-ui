#!/usr/bin/env python3
"""Rendered acceptance for the Glaze UI 2.2 Optical Reachability component layer."""
...
def run_case(
    ...
    *,
    ...
    name = f"2.2 Optical Reachability {width}x{height} {appearance} mode={mode} dir={direction} input={input_mode}"
    ...
    for attempt in range(1, RENDER_ATTEMPTS + 1):
        ...
        with tempfile.TemporaryDirectory(prefix="glaze-22-optical-") as profile:
            ...
            try:
                ...
                if attempt < RENDER_ATTEMPTS:
                    print(f"Glaze UI 2.2 Optical Reachability acceptance retrying: {name}")
                    continue
                break
        ...
        if completed.returncode != 0:
            ...
            print(f"Glaze UI 2.2 Optical Reachability rendered acceptance passed: {name}")
            return
        elif status == "fail":
            raise SystemExit(
                f"Glaze UI 2.2 Optical Reachability rendered acceptance failed for {name}:\n{text or '(no result text)'}"
            )
        else:
    ...
    raise SystemExit(
        f"Glaze UI 2.2 Optical Reachability rendered acceptance failed for {name} after {RENDER_ATTEMPTS} attempts:\n{last}"
    )
...
def main() -> None:
    ...
    print("Glaze UI 2.2 Optical Reachability component rendered acceptance passed")
