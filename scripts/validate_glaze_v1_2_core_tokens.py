...
UNESTABLISHED={'formFactor'};RAW_VALUE=re.compile(r'#[0-9a-fA-F]{3,8}|rgba?\(|\b\d+(?:\.\d+)?(?:px|rem|em|ms)\b')
...
def main():
    ...
    for marker in ('V1.1 / `1.1.0`','glaze-v1.2-core.candidate.json','non-consumer-eligible','does not duplicate raw token values','glaze-v1.2-states.candidate.json','form-factor token ownership remains explicitly unestablished'):require(marker in readme,f'token authority documentation missing marker: {marker}')
