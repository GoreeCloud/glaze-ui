#!/usr/bin/env python3
"""Fail-closed Glaze UI 1.5 Stable form-factor validation."""
...
def main():
    ...
    req(text('VERSION').strip()=='1.5.0','VERSION must be 1.5.0')
    data=json.loads(text('tokens/glaze.tokens.json')); req(data['meta']['version']=='1.5.0' and data['meta']['status']=='Stable','token metadata must declare 1.5.0 Stable'); req(data['meta']['stableBaseline']=='1.5.0','token Stable baseline must be 1.5.0')
    ...
    req('Glaze UI 1.5.0 is the current Stable canonical baseline' in readme,'README 1.5 Stable statement missing')
    ...
    req('Glaze UI 1.5 Stable systems' in lifecycle,'1.5 Stable lifecycle promotion missing')
    req('Stable baseline:** Glaze UI **1.5.0' in text('STABILITY.md'),'1.5 stability baseline missing')
    req(json.loads(text('consumers/registry.json'))['stableBaseline']=='1.5.0','1.5 consumer baseline missing')
    req('Glaze UI 1.5 Stable' in text('website/index.html'),'public site 1.5 Stable marker missing')
    print('Glaze UI 1.5 Stable form-factor source validation passed')
