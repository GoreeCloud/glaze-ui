#!/usr/bin/env python3
"""Rendered acceptance for bounded Glaze UI 2.2 Structure components.

This validates reference-browser behavior for Card, List, Table, Tabs,
Sidebar, Navigation Rail, Dock, and Toolbar. It does not establish native
acceptance, downstream adoption, full component-library completion, human
Visual Excellence approval, or Stable promotion.
"""
...
PAGE = "candidate-2.2-structure-acceptance.html"
...
def run_case(
    ...
    *,
    ...
    params = {"appearance": appearance, "mode": mode, "direction": direction, "input": input_mode}
    name = f"2.2 Structure {width}x{height} {appearance} mode={mode} dir={direction} input={input_mode}"
    ...
    for attempt in range(1, RENDER_ATTEMPTS + 1):
        ...
        with tempfile.TemporaryDirectory(prefix="glaze-22-structure-") as profile:
            ...
            try:
                ...
                if attempt < RENDER_ATTEMPTS:
                    print(f"Glaze UI 2.2 Structure acceptance retrying: {name}")
                    continue
                break
        ...
        if completed.returncode != 0:
            ...
            print(f"Glaze UI 2.2 Structure rendered acceptance passed: {name}")
            return
        elif status == "fail":
            raise SystemExit(f"Glaze UI 2.2 Structure rendered acceptance failed for {name}:\n{text or '(no result text)'}")
        else:
            last = f"attempt {attempt} did not reach PASS (status={status or 'missing'})\n{text or completed.stdout[-3000:]}"
        if attempt < RENDER_ATTEMPTS:
            print(f"Glaze UI 2.2 Structure acceptance retrying: {name}")

    raise SystemExit(f"Glaze UI 2.2 Structure rendered acceptance failed for {name} after {RENDER_ATTEMPTS} attempts:\n{last}")
...
def main() -> None:
    ...
    print("Glaze UI 2.2 Structure rendered acceptance passed")
