...
def main() -> None:
    ...
    form_factors = properties.get("target", {}).get("properties", {}).get("form_factors", {}).get("items", {}).get("enum", [])
    if "wearable" in form_factors:
        fail("deferred wearable support must not be accepted by the current conformance evidence schema")
