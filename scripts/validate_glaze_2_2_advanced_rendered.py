#!/usr/bin/env python3
"""Rendered acceptance for Glaze UI 2.2 Signature + Intelligence components.

Passing is bounded Candidate evidence for the ten advanced components only. It
does not establish full-release rendered acceptance, native/device acceptance,
migration/performance evidence, Human Visual Excellence, consumer eligibility,
or Stable promotion.
"""
...
from validate_rendered_reference import (
    RENDER_ATTEMPTS, acceptance_result, browser_command, find_browser, run_browser, serve_root,
)
...
def run_case(browser: str, port: int, *, width: int, height: int, appearance: str="light",
    ...
    name=f"2.2 Advanced {width}x{height} {appearance} mode={mode} dir={direction} input={input_mode}"
    ...
    for attempt in range(1, RENDER_ATTEMPTS+1):
        ...
        with tempfile.TemporaryDirectory(prefix="glaze-22-advanced-") as profile:
...
def main() -> None:
    ...
    print("Glaze UI 2.2 Signature + Intelligence rendered acceptance passed (Candidate lifecycle enforced)")
