#!/usr/bin/env python3
"""Run rendered acceptance for Glaze UI 1.6 Candidate evidence presentation."""
...
def run_candidate_case(
    ...
    *,
    ...
    url = f"http://127.0.0.1:{port}/reference/candidate-1.6-evidence-acceptance.html?{query}"
    case_name = f"candidate16-evidence {form_factor} {width}x{height} {theme} {mode} text{str(text_scale)}"
    ...
    for attempt in range(1, attempts + 1):
        with tempfile.TemporaryDirectory(prefix="glaze-candidate16-evidence-render-") as profile_dir:
            ...
            try:
                ...
                if attempt < attempts:
                    print(f"Glaze UI 1.6 evidence rendered acceptance retrying: {case_name}")
                    continue
                break
        ...
        if completed.returncode != 0:
            ...
            print(f"Glaze UI 1.6 evidence rendered acceptance passed: {case_name}")
            return
        elif status == "fail":
    ...
    raise SystemExit(
        f"Glaze UI 1.6 Candidate evidence rendered acceptance failed for {case_name} "
...
def main() -> None:
    ...
    print("Glaze UI 1.6 Candidate evidence presentation rendered acceptance passed")
