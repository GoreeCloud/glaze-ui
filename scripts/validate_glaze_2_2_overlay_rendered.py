...
PAGE = "candidate-2.2-overlay-acceptance.html"
...
def run_case(
    ...
    *,
    ...
    name = f"2.2 Overlay {width}x{height} {appearance} mode={mode} dir={direction} input={input_mode}"
    ...
    for attempt in range(1, RENDER_ATTEMPTS + 1):
        ...
        with tempfile.TemporaryDirectory(prefix="glaze-22-overlay-") as profile:
            ...
            try:
                ...
                if attempt < RENDER_ATTEMPTS:
                    print(f"Glaze UI 2.2 Overlay acceptance retrying: {name}")
                    continue
                break
        ...
        if completed.returncode != 0:
            ...
            print(f"Glaze UI 2.2 Overlay rendered acceptance passed: {name}")
            return
        elif status == "fail":
            raise SystemExit(f"Glaze UI 2.2 Overlay rendered acceptance failed for {name}:\n{text or '(no result text)'}")
        else:
            last = f"attempt {attempt} did not reach PASS (status={status or 'missing'})\n{text or completed.stdout[-3000:]}"
        if attempt < RENDER_ATTEMPTS:
            print(f"Glaze UI 2.2 Overlay acceptance retrying: {name}")

    raise SystemExit(f"Glaze UI 2.2 Overlay rendered acceptance failed for {name} after {RENDER_ATTEMPTS} attempts:\n{last}")
