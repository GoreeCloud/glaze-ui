#!/usr/bin/env python3
"""Prepare an exact-source local root for Glaze UI V1.6 human review.

This helper materializes only the frozen V1.6 JavaScript source plus the
current human-review harness. It does not create human evidence or acceptance.
"""
...
HARNESS = Path("reference/v1.6/human-qualification.html")
SENTINEL = ".glaze-v1.6-human-review-root"
...
def prepare(repo_root: Path, output: Path, replace: bool) -> dict:
    ...
    (output / SENTINEL).write_text(
        "Glaze UI V1.6 exact-source human review root\n",
        encoding="utf-8",
    )
    ...
    manifest = {
        "schemaVersion": 1,
        "recordType": "glaze-v1.6-human-review-source",
...
def main() -> None:
    ...
    parser.add_argument("--out", default="_v16-human-review")
    parser.add_argument("--replace", action="store_true")
    args = parser.parse_args()

    manifest = prepare(Path(args.repo_root), Path(args.out), args.replace)
    print("GLAZE UI V1.6 human review root prepared")
    ...
    print("Then open:")
    print("  http://127.0.0.1:8766/reference/v1.6/human-qualification.html")
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit("GLAZE UI V1.6 human review preparation FAILED: " + str(error))
