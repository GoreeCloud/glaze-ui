...
LAUNCHER_CURRENT_ADOPTION = "4043895afd26991d0a20461e94d002f91c890703"
KEYBOARD_CURRENT_ADOPTION = "e56ec4a01dde4024aa5ef54b3d13fd681ef8ada7"
CARD = re.compile(
...
def main() -> int:
    ...
    req(launcher is not None and launcher.get("status") == "adoption-candidate" and launcher.get("targetVersion") == stable, "Launcher must remain the verified 2.2 Adoption Candidate")
    req(launcher.get("referenceRevision") == LAUNCHER_CURRENT_ADOPTION, "Launcher exact source-evidence revision drift")
    req(launcher.get("evidence") == "docs/glaze-ui-adoption.md" and launcher.get("automatedContractPath") == "scripts/check_glaze_ui.py", "Launcher source-verification contract drift")
    req(launcher.get("productionEligible") is False, "Launcher must remain production-ineligible")
    ...
    req(keyboard.get("referenceRevision") == KEYBOARD_CURRENT_ADOPTION, "Keyboard exact source-evidence revision drift")
    ...
    req(keyboard.get("productionEligible") is False, "Keyboard must remain production-ineligible")
    ...
    print(
        "Design Center consumer governance validated: canonical registry schema, exact Launcher/Keyboard 2.2 Adoption Candidate states, "
        "source-evidence fields, production boundary, responsive composition, and forced-colors fallback are synchronized."
    )
    return 0
