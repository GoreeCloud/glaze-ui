...
def main() -> int:
    ...
    matrix = final_artifact_acceptance.get("stableSecurityControlMatrix", {})
    require(matrix.get("evaluatedControlCount") == 39, "final security control matrix must evaluate all 39 controls")
    require(matrix.get("passedOrBoundedPassedCount") == 14, "final security control matrix passed-count mismatch")
    require(matrix.get("notApplicableJustifiedCount") == 25, "final security control matrix N/A-count mismatch")
    require(matrix.get("blockedCount") == 0, "final security control matrix must contain zero blocked controls")
    require(matrix.get("unknownCount") == 0, "final security control matrix must contain zero unknown controls")
    require(matrix.get("exceptedCount") == 0, "final security control matrix must contain zero excepted controls")
    require(matrix.get("exceptions") == [], "final security control matrix must contain no security exceptions")
    acceptance_record = final_artifact_acceptance.get("releaseSecurityAcceptanceRecord", {})
    require(acceptance_record.get("finalSecurityGateResult") == "passed", "release security acceptance record final gate must pass")
    require(acceptance_record.get("exceptions") == [], "release security acceptance record must contain no exceptions")
