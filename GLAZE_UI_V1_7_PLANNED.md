...
As of 2026-09-26, the canonical repository contains bounded V1.7 Development foundations through **1.7.0-dev.18**. Historical implementation references remain bound to the plan revision they were built against: dev.1–dev.7 use the earlier v1.0 numbering, dev.8–dev.13 use the prior v1.1 35-section numbering, dev.14 is the first explicitly v1.2-bound source foundation for **Section 22 — Glaze Signature Motion System**, dev.15 adds the bounded **Section 23 — Signature Motion Principles** source layer, dev.16 adds the bounded **Section 24 — Glaze Signature Transition Families** semantic choreography layer, dev.17 adds the bounded **Section 25 — Connected Transformation 2.0** source layer, and dev.18 adds the bounded **Section 26 — Adaptive Composition Motion** source layer. dev.18 governs eight adaptive composition changes—reposition, resize, hierarchy change, movement between panes, merge, separate, reorder, and material-level change—through Glaze Flow, Glaze Fold, and Glaze Material Shift semantics while preserving spatial understanding, task identity, focus/reading order, selection, and navigation context. It requires an authoritative composition change plus a supplied non-empty authoritative composition identity and explicit from/to composition states; missing authority or identity falls back to the immediate final composition rather than inventing continuity. Reduced Motion uses immediate recomposition with focus continuity, and performance pressure may reduce optional interpolation without changing the final composition. dev.14 does **not** establish Section 22 completion, dev.15 does **not** establish Section 23 completion or measured responsiveness acceptance, dev.16 does **not** establish Section 24 completion or rendered/native choreography acceptance, dev.17 does **not** establish Section 25 completion, and dev.18 does **not** establish Section 26 completion, rendered/native/assistive-technology/performance/motion-fatigue/human-review acceptance, any later v1.2 motion section, consumer eligibility, Release Candidate, Anchor, deployment, or production acceptance. The separate Glaze Motion 0.6 lifecycle remains Experimental. dev.13's “v1.1 Section 28 Continuity-Aware Motion” remains historical implementation provenance and must **not** be reinterpreted as v1.2 Section 28 Theme Transition System.
