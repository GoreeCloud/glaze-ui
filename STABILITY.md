...
Stable remains blocked while three recorded controls remain open: authoritative default-branch protection, final Stable release-security acceptance, and the final artifact/provenance/publication boundary. Platform Contract 0.4 shared-library representation, canonical Drive task/changelog reconciliation, the exact-candidate secret/Git-history scan, remediated selected dependency/supply-chain evidence, production-applicability, and required-check protection readiness are verified complete. PR #291 / main `305de8a1fe5cc7670c1e012efcc7fb55a4bcfdd6` made `validate-v1`, `stable-security-evidence`, `stable-qualification-review`, and `platform-contract-0-4` universal uniquely named pull-request gates; live GitHub protection itself remains disabled and therefore unresolved. The selected Android/Wear build-tool graph is clean and exact-head native compatibility passes. The governed finalization order is fail-closed and non-circular: verify branch protection, select the exact merged final candidate, build an unpublished final candidate artifact with checksum/SBOM/provenance, complete final Stable security acceptance against that exact source and artifact, publish only those already accepted bytes under the immutable release identity, read the publication back, and then run the final exact-candidate Stable qualification review. The existing artifact rehearsal remains non-publishing Release Candidate evidence only.
