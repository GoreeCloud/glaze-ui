...
| FR-016 | Build the V1.6 section 98 acceptance matrix, including accessibility, representative rendering, layout stability, performance, offline/degraded behavior, privacy/authority boundaries, and regression coverage. | P0 | Grouped 24-lane matrix is implemented against frozen revision `c7509c79256b04b0aa67cb9dd0737d7588e0ae4a`. Machine run `35345027816`, rendered-browser run `35348330585`, and rendered-regression run `35352215130` now verify 17 lanes; 7 remain unverified. Remaining partial lanes are Accessibility, Keyboard navigation, Privacy boundaries, Authority boundaries, and Representative rendering; Assistive technology and Performance have no qualifying evidence yet. Fail-closed exact-source control planes now exist for representative Performance measurement, human review of the five human-capable open lanes, and a separate real-session Assistive Technology review. Fail-closed durable-record templates/schemas govern future human, assistive-technology, and performance evidence acceptance, and a new exact-source intake reconciler validates the three current canonical evidence records plus any separately accepted external records through the same 24-lane matrix. Its default state is required to reproduce 17 verified / 7 unverified, and its completion gate stays red until all blockers have real evidence. A coordinated non-authorizing remaining-qualification bundle now prepares the Human, Assistive Technology, and representative Performance review surfaces together under one exact-source/tooling manifest and copies only fail-closed working templates, reducing reviewer setup drift without changing evidence requirements. None of these controls manufactures qualifying evidence or changes the current 17/7 disposition; Assistive Technology and Performance still have no qualifying evidence. |
