...
| FR-016 | Build the V1.6 section 98 acceptance matrix, including accessibility, representative rendering, layout stability, performance, offline/degraded behavior, privacy/authority boundaries, and regression coverage. | P0 | Matrix definition is implemented and hardened as 24 fail-closed exact-revision lanes with required evidence groups. Partial evidence cannot satisfy a multi-group lane. Actual rendered/human, assistive-technology, device/form-factor, performance, and other required evidence remains open and must target one exact governed qualification revision. |
