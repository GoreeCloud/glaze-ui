...
| FR-015 | Expand V1.6 beyond loading into the full semantic state grammar, accessibility profiles, focus, motion families, materials, typography, density, responsive behavior, recovery, diagnostics, and conformance metadata. | P0 | Development source representation now covers planned sections 1–100 continuously through aggregate `1.6.0-dev.11`, including the final acceptance-control matrix, eight core pillars, and governing principle. This is Development source completeness only; it does not establish qualification evidence closure, Candidate, Stable, consumer, deployment, or production acceptance. |
| FR-016 | Build the V1.6 section 98 acceptance matrix, including accessibility, representative rendering, layout stability, performance, offline/degraded behavior, privacy/authority boundaries, and regression coverage. | P0 | Matrix definition implemented in Development as 24 fail-closed exact-revision lanes with an unverified durable acceptance record. Evidence collection/review remains open: representative rendering, assistive-technology, device/form-factor, performance, human review, and any other applicable lane must be satisfied by actual exact-revision evidence before governed qualification review. |
